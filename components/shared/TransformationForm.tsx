...
const TransformationForm = ({data=null, action, userId, type, creditBalance, config=null}: TransformationFormProps) => {
  ...
  useEffect(() => {
    if (image && (type==="restore" || type==="removeBackground")) {setNewTransformation(transformationType.config)}
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
      ...
      {(type==="remove" || type==="recolor") && (
        <div className="prompt-field">
          ...
          control={form.control}
          name="prompt"
          formLabel={ type==='remove' ? "Object to Remove" : "Object to Recolour"}
          className="w-full"
          render={({field}) => (
            <Input
            value={field.value}
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          {isTransforming ? 'Transforming Your Image...' : 'Apply Transformation'}
        </Button>
        ...
        >
          {isSubmitting ? "Submitting Your Image..." : "Save Image"}
        </Button>
      </div>
    </form>
  </Form>
